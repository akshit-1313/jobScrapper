...
import { SlidersHorizontal, Globe, Plus, X, Loader2, Check } from 'lucide-react'
...
/** Shared dirty/saved indicator. */
function SaveState({ isDirty }: { isDirty: boolean }) {
    return (
        <span
...
/**
 * Canonical editor for Search Parameters and Job Sources.
 *
 ...
 *
 * Rendered as TWO cards so each can be linked directly (#search-parameters and
 * #job-sources), but deliberately ONE component with ONE state and ONE action:
 * both live on the same `candidate_preferences` row, so either Save button
 * persists everything currently on screen. Splitting them into two actions
 * would duplicate the write path and let one card silently overwrite unsaved
 * edits in the other.
 *
...
export function SearchParametersPanel({ initialValues, availableSources }: Props) {
    ...
    return (
        <>
        {/* ── Search Parameters ── */}
        <section id="search-parameters" className="scroll-mt-24 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <section id="job-sources" className="scroll-mt-24 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex gap-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600">
                    ...
                    <div>
                        <h3 className="font-semibold text-slate-900">Job Sources</h3>
                        <p className="mt-1 max-w-xl text-sm text-slate-500">
                            Which job boards to search. Only boards on the global allow-list can be
                            selected, and the server applies that list again at run time.
                        </p>
                    </div>
                </div>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
                <p className="text-xs text-slate-500">
                    Saved together with your search parameters — both live on one record.
                </p>
                {saveButton}
            </div>
        </section>
